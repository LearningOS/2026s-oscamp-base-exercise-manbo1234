...
pub fn double_in_thread(numbers: Vec<i32>) -> Vec<i32> {
    let handle = thread::spawn(move || numbers.into_iter().map(|n| n * 2).collect());
    handle.join().unwrap()
}
...
pub fn parallel_sum(a: Vec<i32>, b: Vec<i32>) -> (i32, i32) {
    let handle_a = thread::spawn(move || a.iter().copied().sum());
    let handle_b = thread::spawn(move || b.iter().copied().sum());
...
pub fn scoped_slice_sum(a: &[i32], b: &[i32]) -> (i32, i32) {
    thread::scope(|scope| {
        let sum_a = scope.spawn(|| a.iter().copied().sum::<i32>());
        let sum_b = scope.spawn(|| b.iter().copied().sum::<i32>());
