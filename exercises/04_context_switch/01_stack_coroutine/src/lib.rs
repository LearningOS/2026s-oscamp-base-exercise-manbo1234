...
//! - Callee-saved: `sp`, `ra`, `s0`鈥揱s11`. The `ret` instruction is `jalr zero, 0(ra)`.
...
/// `sp` at 0, `ra` at 8, then `s0`鈥揱s11` at 16, 24, 鈥?104.
#[repr(C)]
...
impl TaskContext {
    ...
    /// - Leave `s0`鈥揱s11` zero; they will be loaded on switch.
...
///
/// In asm: store `sp`, `ra`, `s0`鈥揱s11` to `[a0]` (old), load from `[a1]` (new), zero `a0`/`a1` so we do not leak pointers into the new context, then `ret`.
///
